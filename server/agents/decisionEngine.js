...
class DecisionEngine {
  constructor(cfg = {}, db = null) {
    ...
    this.listeners = [];
    // Load persisted decisions async (non-blocking)
    if (this.db) {
      this.db.all('SELECT decision_json FROM agent_decisions ORDER BY created_at ASC')
        .then(rows => {
          this.decisions = rows.map(r => { try { return JSON.parse(r.decision_json); } catch { return null; } }).filter(Boolean);
        })
  ...
  recordDecision(d) {
    ...
    if (this.db) {
      this.db.run(
        'INSERT INTO agent_decisions (id, agent_role, agent_category, task, tier, status, reasoning, action, guardrails, decision_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [d.id, d.agentRole, d.agentCategory, d.task, d.tier, d.status, d.reasoning, d.action, JSON.stringify(d.guardrailsApplied), JSON.stringify(d), d.recordedAt]
  ...
  resolveDecision(id, approved, feedback = '') {
    ...
    if (d) {
      ...
      d.ownerFeedback = feedback;
      d.resolvedAt = new Date().toISOString();
      this._notify('resolved', d);
      if (this.db) {
        this.db.run(
          'UPDATE agent_decisions SET status=?, owner_feedback=?, resolved_at=?, decision_json=? WHERE id=?',
          [d.status, d.ownerFeedback, d.resolvedAt, JSON.stringify(d), d.id]
        ).catch(() => {});
      }
    }
    return d;
